...
def on_start_game(data):
    ...
    emit('game_started', {"message": "Game started!", "time_limit": room.time_limit}, to=room_id)
...
def on_move(data):
    ...
    if all(all(cell != 0 for cell in row) for row in gs.current_board):
        if gs.current_board == gs.solution:
            ...
            room.game_started = False
            leaderboard = _get_player_info(room)
            emit('game_over', {"leaderboard": leaderboard, "message": f"{player.name} solved the puzzle!"}, to=room_id)
